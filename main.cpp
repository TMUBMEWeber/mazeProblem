// final project : maze problem using DS - stack
...
constexpr auto seed(){ // construct for a random seed
    uint64_t shifted = 0;// uint64_t is as unsigned long long
...
struct PCG{//Permuted congruential generator(同餘置換產生器）to contruct a pseudo random number
...
constexpr auto distribution(Gen &g, T min, T max)
{
  ...
  const auto bias_remainder = numeric_limits<T>::max() % range;
  const auto unbiased_max = numeric_limits<T>::max() - bias_remainder - 1;
  auto r = g();
...
struct Stack{
    ...
    constexpr T pop_back(){//move the top of the stack
...
constexpr Array2D<Cell, num_cols, num_rows> make_maze(){// create a function to create a maze
    PCG pcg;//object of random
    Array2D<Cell, num_cols, num_rows> M;//object of array
    //starting creating point(row, col)
    size_t c = 0;
    size_t r = 0;
...
int main()
{
    ...
    for (size_t row = 0; row < num_rows*2+1; ++row){
        for (size_t col = 0; col < num_cols*2+1; ++col){
            const auto square = [&](){
